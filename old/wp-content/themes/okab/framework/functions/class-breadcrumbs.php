...
class dima_Breadcrumbs_Class {
		...
		switch ( true ) {
			case ( is_home() ):
				...
				if ( $page_for_posts ) {
					...
					$title = esc_html__( 'Latest Posts', 'dima-noor' );
				}
				break;
			case ( is_search() ):
				$title = sprintf( esc_html__( 'Search results for: %s', 'dima-noor' ), get_search_query() );
				break;
			case ( is_404() ):
				$title = esc_html__( 'Error 404 Page', 'dima-noor' );
				break;
			case ( class_exists( 'WooCommerce' ) && ( dima_helper::dima_is_product() || dima_helper::dima_is_shop() ) && ! is_search() ):
				if ( ! dima_helper::dima_is_product() ) {
					...
					if ( $title == '' ) {
						$title = esc_html__( 'Shop', 'dima-noor' );
					}
				}
				break;
			...
			case ( is_archive() && ! dima_helper::dima_is_bbpress() && ! is_search() ):
				if ( is_day() ) {
					$title = sprintf( esc_html__( 'Daily Archives: %s', 'dima-noor' ), '<span>' . get_the_date() . '</span>' );
				} else if ( is_month() ) {
					$title = sprintf( esc_html__( 'Monthly Archives: %s', 'dima-noor' ), '<span>' . get_the_date( _x( 'F Y', 'monthly archives date format', 'dima-noor' ) ) . '</span>' );
				} elseif ( is_year() ) {
					$title = sprintf( esc_html__( 'Yearly Archives: %s', 'dima-noor' ), '<span> ' . get_the_date( _x( 'Y', 'yearly archives date format', 'dima-noor' ) ) . '</span>' );
				} elseif ( is_author() ) {
	...
	public function dima_get_title_breadcrumb_list( $position, $html_out ) {
		$line    = '';
		...
		$output = '' . $line . ''
		          . '<div class="dima-breadcrumbs breadcrumbs-' . esc_attr( $position ) . ' text-' . esc_attr( $position ) . '">
						<span>' . wp_kses( $html_out, $dima_allowed_HTML_tags ) . '</span>
					 </div>';
		...
		$this->html_output = '
			<section class="title_container ' . $floating_output . $this->options['breadcrumbs_position'] . '-style' . '"' . '>
				<div class="page-section-content overflow-hidden">
					...
					' . $this->dima_get_title_breadcrumb_list( $position, $this->html_output ) . '
					</div>
				</div>
			</section>
			';
