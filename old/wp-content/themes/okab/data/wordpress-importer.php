...
if ( class_exists( 'WP_Importer' ) ) {
	class WP_Import extends WP_Importer {
		...
		function import_end() {
			...
			echo '<p>' . esc_html__( 'All done.', 'wordpress-importer' ) . ' <a href="' . admin_url() . '">' . __( 'Have fun!', 'wordpress-importer' ) . '</a>' . '</p>';
		...
		function handle_upload() {
			...
			if ( isset( $file['error'] ) ) {
				...
				echo '<p><strong>' . esc_html__( 'Sorry, there has been an error.', 'wordpress-importer' ) . '</strong><br />';
				printf( esc_html__( 'The export file could not be found at %s. It is likely that this was caused by a permissions problem.', 'wordpress-importer' ), esc_html( $file['file'] ) );
				echo '</p>';
			...
			<form action="<?php echo admin_url( 'admin.php?import=wordpress&amp;step=2' ); ?>" method="post">
				...
				<?php if ( $this->allow_fetch_attachments() ) : ?>
					...
					<p>
						...
						<label
							for="import-attachments"><?php esc_html_e( 'Download and import file attachments', 'wordpress-importer' ); ?></label>
					</p>
				<?php endif; ?>
		...
		function author_select( $n, $author ) {
			...
			wp_dropdown_users( array(
				'name'            => "user_map[$n]",
				'multi'           => true,
				'show_option_all' => esc_html__( '- Select -', 'wordpress-importer' )
			) );
		...
		function process_menu_item( $item ) {
			...
			if ( ! $args['menu-item-parent-id'] ) {
				$field_list = array( 'status', 'title', 'columns', 'icon', 'background' );
			} else {
				$field_list = array( 'icon' );
			}
		...
		function fetch_remote_file( $url, $post ) {
			...
			if ( $code !== 200 ) {
				...
				return new WP_Error(
					'import_file_error',
					sprintf(
						esc_attr__( 'Remote server returned %1$d %2$s for %3$s', 'okab' ),
						$code,
			...
			if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
				...
				return new WP_Error( 'import_file_error', esc_attr__( 'Remote file is incorrect size', 'okab' ) );
			}

			if ( 0 == $filesize ) {
				@unlink( $upload['file'] );

				return new WP_Error( 'import_file_error', esc_attr__( 'Zero size file downloaded', 'okab' ) );
			}
			...
			if ( ! empty( $max_size ) && $filesize > $max_size ) {
				...
				return new WP_Error( 'import_file_error', sprintf( esc_attr__( 'Remote file is too large, limit is %s', 'okab' ), size_format( $max_size ) ) );
			}
