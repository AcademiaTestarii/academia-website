...
<div class="dima-bbp-form">
	<form id="bbp-your-profile" action="<?php bbp_user_profile_edit_url( bbp_get_displayed_user_id() ); ?>"
		...
		<?php foreach ( bbp_edit_user_contact_methods() as $name => $desc ) : ?>
			...
			<p>
				<label
					for="<?php echo esc_attr( $name ); ?>"><?php echo apply_filters( 'user_' . esc_attr( $name ) . '_label', esc_attr( $desc ) ); ?></label>
		...
		<p>
			...
			if ( ! empty( $new_email ) && $new_email !== bbp_get_displayed_user_field( 'user_email', 'edit' ) ) : ?>
				...
				<span class="updated inline">
					<?php printf( wp_kses( __( 'There is a pending email address change to <code>%1$s</code>. <a href="%2$s">Cancel</a>', 'bbpress' ), $dima_allowed_HTML_tags ), $new_email['newemail'], esc_url( self_admin_url( 'user.php?dismiss=' . bbp_get_current_user_id() . '_new_email' ) ) ); ?>
				</span>
		...
		<p>
			...
			<input type="password" name="pass1" id="pass1" size="16" value="" autocomplete="off"
			       tabindex="<?php bbp_tab_index(); ?>"/>
			<span
				class="description"><?php esc_html_e( 'If you would like to change the password type a new one. Otherwise leave this blank.', 'bbpress' ); ?></span>
		</p>
		...
		<p class="bbp-submit-wrapper">
			...
			<button type="submit" tabindex="<?php bbp_tab_index(); ?>" id="bbp_user_edit_submit"
			        ...
			        class="button submit user-submit"><?php esc_html_e( 'Update', 'okab' ); ?></button>
		</p>
