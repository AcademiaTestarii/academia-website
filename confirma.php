...
if (isset($_GET['cheie']) AND strlen($_GET['cheie'])==12 AND ctype_alnum($_GET['cheie'])) {
	...
	$sql="SELECT * FROM `cursanti` WHERE `cod_confirmare`='".$cod_confirmare."' AND `activ`=0";
	$query=mysqli_query($link,$sql);
	if (mysqli_num_rows($query)==1) {
		// activam
		$today=date("Y-m-d H:i:s");
		$row=mysqli_fetch_assoc($query);
		$sql_update="UPDATE `cursanti` SET `activ`=1,`cod_confirmare`=NULL,`activitate`='".$today."' WHERE id=".$row['id'];
