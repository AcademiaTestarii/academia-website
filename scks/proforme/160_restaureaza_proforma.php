...
try {
    $output = $sbcClient->cancelProforma($companyVatCode, $estimateSeries, $estimateNumber);
...
// Restaurare proforma
echo 'Restaurare proforma: ';
try {
    $sbcClient->restoreProforma($companyVatCode, $companyProformaSeries, "1111");
    echo $message = $output['message'];
} catch (Exception $ex) {
    echo $ex->getMessage();
}
// Restaurare proforma: Proforma cu seria si numarul P0194 a fost anulata cu succes. -> cu mesaj de succes
// Restaurare proforma: Proforma cu seria si numarul P1111 nu a fost gasita! -> cu mesaj de eroare
