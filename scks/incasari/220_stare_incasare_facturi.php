...
$payment = array(
    ...
    'precision'		=> 2,
    'value'			=> 20,
    'type'          => SmartBillCloudRestClient::PaymentType_BiletOrdin,
    'isCash'		=> false,
...
echo 'Stare incasare factura: ';
try {
    $output = $sbcClient->statusInvoicePayments($companyVatCode, $invoiceSeries, $invoiceNumber);
    $total  = $output['invoiceTotalAmount'];
    $paid   = $output['paidAmount'];
    $unpaid = $output['unpaidAmount'];
print_r($output);
} catch (Exception $ex) {
    echo $ex->getMessage();
}